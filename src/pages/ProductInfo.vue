...
export default {
  ...
  mounted() {
    axios(
      `https://61922b19aeab5c0017105dfb.mockapi.io/product/${this.$route.params.id}`
    ).then(
    ...
    <main class="row flex-column flex-md-row">
      <div class="col-12 col-md-6 py-3 px-3 animate__animated animate__zoomIn delay-1s">
        <img
          :src="details.image"
          :alt="details.name"
          class="w-100 h-100 rounded rounded-2"
        />
      </div>
      <div class="col-12 col-md-6 pb-4">
        ...
        <div class="h5 mb-5 d-flex flex-row gap-1 animate__animated animate__fadeInDown">
          ...
          >
          <span
            class="text-danger border border-2 border-danger rounded rounded-pill px-2"
            >{{ details.type }}</span
          >
        </div>
